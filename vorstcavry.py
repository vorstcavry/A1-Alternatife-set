...
def custom_download(custom_dirs, user_header, api_key):
    for key, value in custom_dirs.items():
        ...
        if value.url:
            print_line(0)
            cprint(f"[+] Downloading {key}.", color="flat_blue")
...
def download_from_textfile(filename, api_key):
    for key, urls in parse_urls(filename).items():
        ...
        if key_lower in custom_dirs:
            ...
            cprint(f"Warning: Category '{key}' from the file is not found in custom_dirs.", color="flat_blue")
...
if __name__ == "__main__":
    ...
    # variable
    args.req = "https://github.com/vorstcavry/A1-Alternatife-set/raw/main/set/req.txt" 
    ...
    print_line(0)
    cprint(f"PyTorch Version :", torch_ver, "| Cuda :", cuda_ver, "| ", xformers_version, "| GPU :", gpu_status, "| Env :", env, "|", color="flat_yellow")
